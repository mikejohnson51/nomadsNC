...
To do this we can use dataRetrieval::findNLDI!

```{r}
library(dataRetrieval)

# use dataRetrieval::findNLDI to identify your catchment COMID
comid <- findNLDI(location = c(-104.8253, 38.83396))$origin$comid
...
feature_of_interest <- get_vaa("reachcode") %>% 
  filter(comid == max_flow_comid) %>% 
  mutate(huc8 = substr(reachcode, 1, 8))

huc8 <-  get_huc8(id = feature_of_interest$huc8)
nhd  <-  get_nhdplus(AOI = huc8)
...
records <- do.call(rbind, lapply(1:length(comid_index), FUN = function(x){ 
  var.get.nc(nc, "streamflow", start = c(comid_index[x],1), count =c(1, 18), unpack = TRUE) 
}))
